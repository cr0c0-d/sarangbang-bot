 ...
 */
const BOTS = {
  mango: { name: '망고', features: ['tts', 'timer', 'images', 'poll', 'movie', 'plan', 'ai', 'stream', 'voice'] },
...
export const config = {
  ...
  // 🎧 음성채널 소리 되돌리기 (`/음성기록`). **방송 기록과 별개 기능입니다.**
  // ⚠️ 실제 디스코드 수신을 아직 확인하지 못해 **기본 꺼짐**입니다.
  //    `/관리자 음성수신확인` 으로 먼저 확인하세요. docs/음성녹음-기획.md
  voice: {
    enabled: bool('VOICE_RECORD', false),
    // 되돌릴 길이(초). 링버퍼가 메모리에 들고 있는 양이라 늘리면 메모리도 늘어납니다.
    // 6명 30초 = 약 3.6MB. 통째로 녹음하는 것과 200배 차이입니다.
    clipSec: Math.max(5, Math.min(120, num('VOICE_CLIP_SEC', 30))),
    // 한 폴더(하루)에 남길 수 있는 개수.
    // ⚠️ 용량 예산은 클립과 **같이** 씁니다(STREAM_CLIP_MAX_GB) — 따로 두면 정리가
    //    소리만 조용히 못 지웁니다. 개수만 따로 셉니다.
    maxClips: Math.max(1, num('VOICE_MAX_CLIPS', 30)),
    // ★ 사람이 들어오면 **따라 들어가 자동으로 켜는** 음성채널 목록.
    // ⚠️ **비워두면 자동으로 켜지지 않습니다.** 이미지 채널(비우면 전체)과 **반대**입니다 —
    //    남의 대화를 아무 방에서나 듣게 두지 않기 위해서입니다. 관리자가 방을 골라야 합니다.
    //    `/관리자 채널설정 종류:소리 기록 음성채널` 로 지정하는 편이 쉽습니다.
    autoChannelIds: list('VOICE_AUTO_CHANNEL_ID'),
    // 몇 명부터 켤지. 혼자 있을 때 켜는 것은 의미도 없고 기분도 나쁩니다.
    autoMinPeople: Math.max(1, num('VOICE_AUTO_MIN_PEOPLE', 2)),
  },
