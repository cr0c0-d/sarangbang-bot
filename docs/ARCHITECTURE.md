...
0. **일정 관리 · 정산** — 기획은 끝났고 코드가 없다.
   [docs/일정-정산-기획.md](일정-정산-기획.md) 를 먼저 읽을 것.
   **일정 하나 = 채널 하나** 로 잡았다(소유자가 이미 그렇게 쓰고 있다).
   채널 ID 가 곧 일정 ID 이고, 채널 권한이 곧 참여자 목록이다.
   날짜 조율·자동 감지는 **의도적으로 넣지 않았다** (소유자: "과하다").
