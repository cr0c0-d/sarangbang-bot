...
0. **일정 관리 · 여행 계획** — 기획만 되어 있고 코드가 없다.
   [docs/일정-여행-기획.md](일정-여행-기획.md) 를 먼저 읽을 것. 요약하면 요청 두 개가
   **부품 두 개**(복수 선택 날짜 조율 · 체크리스트)로 환원되고, 알림은 타이머를 재사용한다.
   그 문서 8절의 결정 사항이 정해지기 전에는 시작하지 말 것.
