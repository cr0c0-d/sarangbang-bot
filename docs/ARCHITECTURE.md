...
### 3.6-8 영화 고르기 (`src/movie/`) — **설계만 되어 있고 아직 구현 안 됨**
...
#### provider ID 는 하드코딩하되 **시작할 때 대조한다**
...
0. **영화 고르기 (`/영화`)** — 설계는 3.6-8 에 끝나 있고 **코드가 없다.**
   시작하려면 `TMDB_API_KEY` 가 먼저 필요하다 (소유자가 발급해야 함).
   순서: `movie/tmdb.js`(호출·provider 실측) → 고르기 판 → 뽑기 → 투표 연결 → verify.
