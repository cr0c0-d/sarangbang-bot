...
`restartAtCurrentPosition()` 이 위치를 구해 `toOggOpus` 의 `seekSec` 으로 이어 붙인다.

**함정: `playbackDuration` 만 보면 안 된다.** 이 값은 *지금 리소스*가 재생한 시간만 센다.
한 번 다시 틀면 새 리소스는 0 부터 세므로, **두 번째 음량 조절부터 곡이 처음으로 돌아간다.**
실제로 겪은 버그다. 그래서 건너뛴 만큼을 `currentOffsetSec` 에 남기고
`positionSec() = currentOffsetSec + playbackDuration/1000` 으로 구한다.
`playedNothing()`(재생 실패 판정)도 같은 값을 써야 한다 — 안 그러면 곡 끝 무렵에 음량을 바꿨을 때
멀쩡히 다 들은 곡을 "재생 실패" 로 잘못 알린다.

**버튼 연타는 모아서 한 번만 반영한다**(700ms). `+10` 을 세 번 누르면 세 번 끊긴다.
