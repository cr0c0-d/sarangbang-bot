...
## 2.1 한 코드로 봇 여러 개 돌리기 (`BOT_ROLE`)

기본값은 `all` — **아무것도 안 하면 지금까지와 똑같이 봇 하나가 전부 한다.**
나누고 싶을 때만 켜는 스위치다.

| 역할 | 맡는 기능 | 실행 |
|---|---|---|
| `all` | music, tts, timer, images | `npm start` |
| `music` | music | `npm run start:music` |
| `home` | tts, timer, images | `npm start` (`.env` 에 `BOT_ROLE=home`) |
...
`/도움말` `/기능` `/채널설정` `/음량` 은 **봇마다 따로 설정해야 하므로** 양쪽에 등록된다.
디스코드 명령어 목록에 같은 이름이 두 번 보이고, 봇 아이콘으로만 구분된다.
불편하지만 이게 맞다 — 음악 봇을 끄는 `/기능` 과 나머지 봇을 끄는 `/기능` 은 다른 일이다.
대신 각자 **자기 역할의 항목만** 보여준다.

`verify.mjs` 가 역할마다 실제로 프로세스를 띄워
`music ∪ home = all`, `music ∩ home = 공용 4개` 를 검사한다. 이 불변조건을 깨지 말 것.
