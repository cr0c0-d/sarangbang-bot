...
**곡이 바뀔 때 새 메시지를 보내지 않고 기존 제어판을 수정한다** (`showPanel()`).
`GuildAudio.panelMessage` 가 그 메시지를 들고 있다. 이게 없으면 재생목록 20곡을 돌릴 때
채팅방이 "재생 중" 메시지 20개로 도배된다. 수정이 실패하면(메시지 삭제 등) 새로 보낸다.
...
그래서 인증을 **전역 미들웨어에서 `/api/*` 전용으로 옮겼다.**

- `GET /`, `/f/:folder`, `/img/...`, `/dl/...` → **암호 없음**
- `POST /api/delete`, `/api/move`, `/api/folder` → `WEB_TOKEN` 필요
